...
contract PancakeSwapSmartRouter is Ownable, ReentrancyGuard {
    ...
    event SwapMulti(
        address indexed sender, address indexed srcTokenAddr, address indexed dstTokenAddr, uint256 srcAmount
    );
    ...
    */
    constructor(address _WETHAddress, address _pancakeswapV2, address _stableswapFactory) {
    ...
    function _swapOnStableSwap(IERC20 srcToken, IERC20 dstToken, uint256 amount) internal {
        ...
        IStableSwapFactory.StableSwapPairInfo memory info =
            IStableSwapFactory(stableswapFactory).getPairInfo(address(srcToken), address(dstToken));
    ...
    function _swapOnV2ExactIn(IERC20 srcToken, IERC20 dstToken, uint256 amount)
        internal
        returns (uint256 returnAmount)
    {
        if (srcToken.isETH()) {
